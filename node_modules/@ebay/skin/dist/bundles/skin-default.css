...
ul.accordion details .details__content{
  ...
  transition:opacity .5s cubic-bezier(.3, 0, 0, 1),transform .5s cubic-bezier(.3, 0, 0, 1);
}
...
a.fake-btn--secondary,a.fake-btn--secondary:visited,button.btn--secondary{
...
@media (max-width:512px){
  ...
  .lightbox-dialog--fullscreen .lightbox-dialog__handle,.lightbox-dialog--large .lightbox-dialog__handle{
    display:none;
  }
}
@media (min-width:512px){
...
.tooltip__mask{
  background-color:var(--tooltip-background-color, var(--color-background-elevated));
